...
\begin{enumerate}
   ...
   \item \begin{command} cp ../misc/polygons.tar.bz2 . \end{command}
   \item \begin{command} tar -xvf polygons.tar.bz2 \end{command}
   ...
   \item Note that the next command should produce some errors (this is expected
         ), just make sure the error messages all have this message in them:
         \begin{data} duplicate key value violates unique constraint \end{data}
   \item \begin{command} psql polygons -f dump.sql >/dev/null \end{command}
\end{enumerate}
